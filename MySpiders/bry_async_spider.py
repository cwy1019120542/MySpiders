import requests,json,pymysql,chardet,json,pymysql,time,demjson,asyncio,aiohttp,re
from pyquery import PyQuery as pq
from datetime import datetime,timedelta
from apscheduler.schedulers.blocking import BlockingScheduler
db=pymysql.connect(host='127.0.0.1',user='root',passwd='never1019120542,',db='bry_data',charset='utf8')
db.autocommit(True)
cursor=db.cursor()
headers={
...
def to_null(value):
	...
	return value
async def lhb_rank(page,date):
	async with aiohttp.ClientSession() as session:
		async with session.get(f'http://data.eastmoney.com/DataCenter_V3/stock2016/TradeDetail/pagesize=200,page={page},sortRule=-1,sortType=,startDate={date},endDate={date},gpfw=0,js=var%20data_tab_1.html?rt=25754497',headers=headers) as response:
			html=await response.text('gb2312','ignore')
			json_data=html.strip('=qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM 0123456789_')
			#print(json_data)
			...
			if not data_lists:
				...
				for data in data_lists:
					if data['Tdate'] == date:
						...
						cursor.execute(sql2)
						print('当日龙虎榜涨跌幅排名：{}-{}-{}-导入完成'.format(date,data['SCode'],data['SName']))
					else:
						break
				print(f'龙虎榜当日跌幅排名_第{page}页_爬取完成')

async def institution_business(page,date):
	async with aiohttp.ClientSession() as session:
		async with session.get(f'http://data.eastmoney.com/DataCenter_V3/stock2016/DailyStockListStatistics/pagesize=50,page={page},sortRule=-1,sortType=PBuy,startDate={date},endDate={date},gpfw=0,js=var%20data_tab_1.html?rt=25754580') as response:
			html=await response.text('gb2312','ignore')
			json_data=html.strip('=qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM 0123456789_')
			#print(json_data)
			data_lists=demjson.decode(json_data)['data']
			if not data_lists:
				...
				for data in data_lists:
					if data['TDate'] == date:
						...
						cursor.execute(sql2)
						print('机构买卖情况：{}-{}-{}-导入完成'.format(date,data['SCode'],data['SName']))
					else:
						break
				print(f'机构买卖情况_第{page}页_爬取完成')
...
if __name__ == '__main__':
	...
	tasks=[lhb_rank(page,date) for page in range(1,3)]+[institution_business(page,date) for page in range(1,3)]
